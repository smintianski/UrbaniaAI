...
# LOAD DATA
# ----------------------------------------------------------------------------------------------------------------------
@st.cache_data(ttl=3600, show_spinner="Loading Valencia data...")
def load_all_data():
...
with col_ui:
    st.title("🗺️ Route Planner Valencia")
    ...
    # Add a button to force route calculation
    calculate_route = st.button("Calculate Route", type="primary")
...
with col_ui:
    ...
    # Add some stats about loaded data
    with st.expander("ℹ️ Data Status"):
        st.write(f"🅿️ Parkings loaded: {len(parkings)}")
        st.write(f"🚲 Valenbisi stations: {len(bikes)}")
        st.write(f"🚌 Bus stops: {len(buses)}")
        st.write(f"🚦 Traffic sensors: {len(traffic)}")
