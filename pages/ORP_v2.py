...
def clear_session_data():
    """Clear markers and routes from session state"""
    st.session_state.markers = {}
    st.session_state.routes = {}

def get_session_key(*args):
    """Generate a unique key for session state storage"""
    return str(hash(str(args)))

# ----------------------------------------------------------------------------------------------------------------------
# CACHED HELPERS (only for external data)
# ----------------------------------------------------------------------------------------------------------------------
...
def geocode(address: str):
    """Return (lat, lon) tuple using session state cache"""
    if address not in st.session_state.geocoded_addresses:
        st.session_state.geocoded_addresses[address] = geocode_cached(address)
    return st.session_state.geocoded_addresses[address]
...
def get_route(coords, profile="foot-walking"):
    """Get route using session state storage"""
    route_key = get_session_key(coords, profile)
    
    if route_key not in st.session_state.routes:
        try:
            route_data = ors_client.directions(coordinates=coords, profile=profile, format="geojson")
            st.session_state.routes[route_key] = route_data
        except Exception:
            st.session_state.routes[route_key] = None
    
    return st.session_state.routes[route_key]
...
def add_marker_to_session(coords, tooltip, color, marker_type="default"):
    """Add marker to session state"""
    marker_key = get_session_key(coords, tooltip, color, marker_type)
    marker_data = {
        "coords": coords,
        "tooltip": tooltip,
        "color": color,
    ...
    st.session_state.markers[marker_key] = marker_data
    return marker_key

def get_markers_from_session():
    """Get all markers from session state"""
    return st.session_state.markers
...
def build_map():
    ...
    # Clear previous session data when inputs change
    clear_session_data()
    ...
    # Add start and end markers to session
    add_marker_to_session(start_coords, "Start", "green", "start")
    add_marker_to_session(end_coords, "End", "red", "end")
    ...
    if transport_mode == "Car":
        ...
        if use_parking == "Parking garage":
            ...
            if parking:
                add_marker_to_session(parking["geo_point_2d"], "Nearest Parking Garage", "blue", "parking")
    ...
    # VALENBISI MODE
        ...
        if st_start and st_end:
            add_marker_to_session(st_start["geo_point_2d"], "Bike pickup", "orange", "bike_pickup")
            add_marker_to_session(st_end["geo_point_2d"], "Bike dropoff", "purple", "bike_dropoff")
    ...
    # BUS MODE
        ...
        # Add bus stops to session
        add_marker_to_session(stop_start["geo_point_2d"], f"Start stop · LINE {best_line}", "lightblue", "bus_start")
        add_marker_to_session(stop_end["geo_point_2d"], f"End stop · LINE {best_line}", "lightblue", "bus_end")
    ...
    # Add all markers from session to map
    for marker_data in get_markers_from_session().values():
        folium.Marker(
            marker_data["coords"], 
            tooltip=marker_data["tooltip"],
            icon=folium.Icon(color=marker_data["color"])
        ).add_to(m)
...
with col_ui:
    ...
    with st.expander("Debug Info"):
        st.write(f"Markers in session: {len(st.session_state.markers)}")
        st.write(f"Routes in session: {len(st.session_state.routes)}")
        st.write(f"Geocoded addresses: {len(st.session_state.geocoded_addresses)}")
